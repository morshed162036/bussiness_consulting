...
class AdminController extends Controller
{
    ...
    public function dashboard(){
        $new = Appointment::where('status','Create')->get()->count();
        $complete = Appointment::where('status','Complete')->get()->count();
        $pending = Appointment::where('status','Pending')->get()->count();
        $service = Service::get()->count();
        $setup = Setup::get()->count();
        $category = Blog_category::get()->count();
        $blog = Blog_post::get()->count();
        return view('server.dashboard')->with(compact('new','complete','pending','service','category','blog','setup'));
    }
}
